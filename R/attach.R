...
pkgs <- c("shinysurveys", "ghee")

metajdt_attach <- function() {
  # Create `to_load` which is a character vector of all metajdt
  ...
  # metajdt with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("metajdt ", package_version("metajdt"))
  )

  # Return a character string containing the package version for each of metajdt's constituents
...
#' List all packages imported by metajdt
#'
#' @export
#'
#' @examples
#' metajdt_packages()
metajdt_packages <- function() {
  # get all imports from metajdt's package description file
  raw <- utils::packageDescription("metajdt")$Imports
